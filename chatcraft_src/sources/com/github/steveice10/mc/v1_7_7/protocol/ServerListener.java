...
import com.github.steveice10.mc.v1_7_7.protocol.data.status.handler.ServerInfoBuilder;
        ...
        if (minecraftProtocol.getMode() == ProtocolMode.HANDSHAKE && (packetReceivedEvent.getPacket() instanceof HandshakePacket)) {
            ...
            switch (handshakePacket.getIntent()) {
                ...
                case 2:
                    ...
                    if (handshakePacket.getProtocolVersion() > 5) {
                        packetReceivedEvent.getSession().disconnect("Outdated server! I'm still on 1.7.7.");
                        break;
                    } else if (handshakePacket.getProtocolVersion() < 5) {
                        packetReceivedEvent.getSession().disconnect("Outdated client! Please use 1.7.7.");
                        break;
                    }
                    break;
                default:

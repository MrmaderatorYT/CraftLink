...
import com.github.steveice10.mc.v1_9_2.protocol.data.status.handler.ServerInfoBuilder;
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.HANDSHAKE && (packetReceivedEvent.getPacket() instanceof HandshakePacket)) {
            ...
            switch (handshakePacket.getIntent()) {
                ...
                case LOGIN:
                    ...
                    if (handshakePacket.getProtocolVersion() > 109) {
                        packetReceivedEvent.getSession().disconnect("Outdated server! I'm still on 1.9.2.");
                        break;
                    } else if (handshakePacket.getProtocolVersion() < 109) {
                        packetReceivedEvent.getSession().disconnect("Outdated client! Please use 1.9.2.");
                        break;
                    }
                    break;
                default:

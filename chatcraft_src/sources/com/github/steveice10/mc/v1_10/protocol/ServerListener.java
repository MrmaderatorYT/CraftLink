...
import com.github.steveice10.mc.v1_10.protocol.data.status.PlayerInfo;
import com.github.steveice10.mc.v1_10.protocol.data.status.ServerStatusInfo;
import com.github.steveice10.mc.v1_10.protocol.data.status.VersionInfo;
import com.github.steveice10.mc.v1_10.protocol.data.status.handler.ServerInfoBuilder;
...
import com.github.steveice10.mc.v1_10.protocol.util.CryptUtil;
...
public class ServerListener extends SessionAdapter {
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.HANDSHAKE && (packetReceivedEvent.getPacket() instanceof HandshakePacket)) {
            ...
            switch (handshakePacket.getIntent()) {
                ...
                case LOGIN:
                    ...
                    if (handshakePacket.getProtocolVersion() > 210) {
                        packetReceivedEvent.getSession().disconnect("Outdated server! I'm still on 1.10.");
                        break;
                    } else if (handshakePacket.getProtocolVersion() < 210) {
                        packetReceivedEvent.getSession().disconnect("Outdated client! Please use 1.10.");
                        break;
                    }
                    break;
                default:
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.STATUS) {
            if (packetReceivedEvent.getPacket() instanceof StatusQueryPacket) {
                ...
                if (serverInfoBuilder == null) {
                    serverInfoBuilder = new ServerInfoBuilder() { // from class: com.github.steveice10.mc.v1_10.protocol.ServerListener.1
                        @Override // com.github.steveice10.mc.v1_10.protocol.data.status.handler.ServerInfoBuilder
    ...
    private class UserAuthTask implements Runnable {
        ...
        public void run() {
            ...
            int iIntValue = this.session.hasFlag("compression-threshold") ? ((Integer) this.session.getFlag("compression-threshold")).intValue() : 256;

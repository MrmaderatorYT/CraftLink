...
import com.github.steveice10.mc.v1_9.protocol.data.status.handler.ServerInfoBuilder;
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.HANDSHAKE && (packetReceivedEvent.getPacket() instanceof HandshakePacket)) {
            ...
            switch (handshakePacket.getIntent()) {
                ...
                case LOGIN:
                    ...
                    if (handshakePacket.getProtocolVersion() > 107) {
                        packetReceivedEvent.getSession().disconnect("Outdated server! I'm still on 1.9.");
                        break;
                    } else if (handshakePacket.getProtocolVersion() < 107) {
                        packetReceivedEvent.getSession().disconnect("Outdated client! Please use 1.9.");
                        break;
                    }
                    break;
                default:

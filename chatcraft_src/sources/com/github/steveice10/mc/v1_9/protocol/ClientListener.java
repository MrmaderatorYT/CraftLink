...
import com.github.steveice10.mc.v1_9.protocol.data.handshake.HandshakeIntent;
import com.github.steveice10.mc.v1_9.protocol.data.status.ServerStatusInfo;
import com.github.steveice10.mc.v1_9.protocol.data.status.handler.ServerInfoHandler;
import com.github.steveice10.mc.v1_9.protocol.data.status.handler.ServerPingTimeHandler;
...
import com.github.steveice10.mc.v1_9.protocol.packet.ingame.server.ServerSetCompressionPacket;
...
public class ClientListener extends SessionAdapter {
    ...
    public void packetReceived(PacketReceivedEvent packetReceivedEvent) throws NoSuchAlgorithmException {
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.LOGIN) {
            if (packetReceivedEvent.getPacket() instanceof EncryptionRequestPacket) {
                ...
                GameProfile gameProfile = (GameProfile) packetReceivedEvent.getSession().getFlag("profile");
                String string = new BigInteger(CryptUtil.getServerIdHash(encryptionRequestPacket.getServerId(), encryptionRequestPacket.getPublicKey(), secretKeyGenerateSharedKey)).toString(16);
                String str = (String) packetReceivedEvent.getSession().getFlag("access-token");
                System.out.println("Profile " + gameProfile.getName() + " " + str);
                try {
                    new SessionService(proxy).joinServer(gameProfile, str, string);
    ...
    public void connected(ConnectedEvent connectedEvent) {
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.LOGIN) {
            ...
            connectedEvent.getSession().send(new HandshakePacket(107, connectedEvent.getSession().getHost(), connectedEvent.getSession().getPort(), HandshakeIntent.LOGIN));
        ...
        if (minecraftProtocol.getSubProtocol() == SubProtocol.STATUS) {
            minecraftProtocol.setSubProtocol(SubProtocol.HANDSHAKE, true, connectedEvent.getSession());
            connectedEvent.getSession().send(new HandshakePacket(107, connectedEvent.getSession().getHost(), connectedEvent.getSession().getPort(), HandshakeIntent.STATUS));

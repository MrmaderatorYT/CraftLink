package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* loaded from: classes.dex */
public class axc extends axb {
    private static final Object s = new Object();
    private static final String t = axc.class.getSimpleName();
    private static boolean u = false;
    private static long v = 0;
    protected boolean r;
    private String w;
    private boolean x;
    private boolean y;
    private ayc z;

    protected static synchronized void a(Context context, boolean z) {
        if (!u) {
            v = System.currentTimeMillis() / 1000;
            f2361a = b(context, z);
            u = true;
        }
    }

    protected axc(Context context, String str, boolean z) {
        super(context);
        this.r = false;
        this.x = false;
        this.y = false;
        this.w = str;
        this.r = z;
    }

    private static axt b(Context context, boolean z) {
        if (f2361a == null) {
            synchronized (s) {
                if (f2361a == null) {
                    axt axtVarA = axt.a(context, "qDz6YvDkhwdxUOtNXedEKNdh2XDWXqUECYckxUUtMRo=", "", z);
                    if (axtVarA.b()) {
                        if (((Boolean) bra.e().a(o.bl)).booleanValue()) {
                            axtVarA.a("phpjyfBMe8u7C5auGMsy22WXoT6iMDb5qqttOP4sXOBlc73QdE1wdYLJ++PsHndY", "86B2wkBiSbEIJu45HO/BQ/RpWXZRq9YpFeIER87ao7I=", new Class[0]);
                        }
                        axtVarA.a("DPtJycwRqj/e0TdTHULzeUhZhWJ1IU3iwhH90sUbn4eYKEdB5HI7UC0PtJgg2RSN", "Ye7G7hL63+8nOBoyCAHdjfK62rvCOKz3+aC1KA/K9CI=", Context.class);
                        axtVarA.a("1IkHdmwdwv9wD8tmJQC+h66q1PQxvS+V9UrDno8I9cxHFnF0E43buTrJO+G+/gQl", "A9jthiAQc+izVBr4mJNKEZTv7NXyHy0tI8Qg667eCzo=", Context.class);
                        axtVarA.a("2a/4JIVPkDFYMOVDvRgiPhzuBvaMjATDrq7QEolObsd/NV02/K8EoZgGgGExyTEN", "zAquriPjpzqGJfvs/9ulqqLxOML+UEqSlyjOz6Tu5VU=", Context.class);
                        axtVarA.a("5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", Context.class);
                        axtVarA.a("tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", Context.class);
                        axtVarA.a("M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", Context.class);
                        axtVarA.a("ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", Context.class);
                        axtVarA.a("DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", Context.class);
                        axtVarA.a("LOklmrwaQ0v3CGeP+ZosklY8SbmufmvNG6auvqsL+bGnLcr54FaTv3bt+awlDR0E", "/Fj70v9BUIdYkRnDi6EwnKWPurvJ9lYtBhSoj7eKwt8=", MotionEvent.class, DisplayMetrics.class);
                        axtVarA.a("BDp2p9FkJRWhLjM5/HGlBtfI2hTpjdS5yJQGMqe31LtNjFTgL+0QOyyChfmPPFl0", "got2az0DwXyMWaLpfVutWfEF1l3X50FJhmqzREDcTA8=", MotionEvent.class, DisplayMetrics.class);
                        axtVarA.a("teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", new Class[0]);
                        axtVarA.a("nm3YsDf+7WO+MEOswfwP/mr6SZsOgwQFOzWfB0E01vRE8SnBNpESCxeGx/E3obzH", "ntbFRCAexgxz9p8O68TxIQjW9W7nZKRNbG91vq8F9ro=", new Class[0]);
                        axtVarA.a("OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", new Class[0]);
                        axtVarA.a("55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", new Class[0]);
                        axtVarA.a("diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", new Class[0]);
                        axtVarA.a("HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", new Class[0]);
                        axtVarA.a("qsalJ2RwJdF4/xIua4a94Fnnrn+PF/pR/qVYX8QPAaTQDa6NRA9etEnbnhQWblFD", "G68A6YJ+VRJhgHK56Kx1RoGHOyqehPG7tlyyPwtcr7Q=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        axtVarA.a("dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=", StackTraceElement[].class);
                        axtVarA.a("JpT53kCFZ5UGhS3QgNfIxU7Zo+2b3P+3vnYveTkMBHt4zfnotqnDwQTaAD1ryBW9", "SQmHWbTAzPlYuz5I3xZRZNLM/2+IGX009ow9MnjYfrI=", View.class, DisplayMetrics.class);
                        axtVarA.a("9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", Context.class, Boolean.TYPE);
                        axtVarA.a("TxRdKOZ0pZeBXpIwiLITX81ZqIx50eBVN3DINE4ZBUPdk4novnMzQn3dOLT7/176", "m2+gcfdQWoY+6hOETk2mr0SsbjGMFJBzTmSItOYkEfM=", View.class, Activity.class, Boolean.TYPE);
                        axtVarA.a("9MXEPmGC/7986z/wLWv2b+1iRSmCMoPNrO7Sy/hYl1riiNMJKXOd189OeOHpltpa", "DjhY2o1Svq1HpINv7cU+bAqV+OA81bMZ6vkVBk0u3II=", Long.TYPE);
                    }
                    f2361a = axtVarA;
                }
            }
        }
        return f2361a;
    }

    private static axz a(axt axtVar, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzdi {
        Method methodA = axtVar.a("LOklmrwaQ0v3CGeP+ZosklY8SbmufmvNG6auvqsL+bGnLcr54FaTv3bt+awlDR0E", "/Fj70v9BUIdYkRnDi6EwnKWPurvJ9lYtBhSoj7eKwt8=");
        if (methodA == null || motionEvent == null) {
            throw new zzdi();
        }
        try {
            return new axz((String) methodA.invoke(null, motionEvent, displayMetrics));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdi(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.axb
    protected final akp a(Context context, aho ahoVar) throws InterruptedException {
        akp akpVar = new akp();
        if (!TextUtils.isEmpty(this.w)) {
            akpVar.f2039b = this.w;
        }
        axt axtVarB = b(context, this.r);
        if (axtVarB.c() != null) {
            a(a(axtVarB, context, akpVar, (aho) null));
        }
        return akpVar;
    }

    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r1v47, types: [java.util.List] */
    @Override // com.google.android.gms.internal.ads.axb
    protected final akp a(Context context, View view, Activity activity) throws InterruptedException {
        ArrayList arrayList;
        Long lValueOf;
        akp akpVar = new akp();
        if (!TextUtils.isEmpty(this.w)) {
            akpVar.f2039b = this.w;
        }
        axt axtVarB = b(context, this.r);
        if (!axtVarB.b()) {
            akpVar.k = 16384L;
            arrayList = Arrays.asList(new ayo(axtVarB, akpVar));
        } else {
            try {
                axz axzVarA = a(axtVarB, this.f2362b, this.q);
                akpVar.g = axzVarA.f2395a;
                akpVar.h = axzVarA.f2396b;
                akpVar.i = axzVarA.c;
                if (this.p) {
                    akpVar.u = axzVarA.d;
                    akpVar.v = axzVarA.e;
                }
            } catch (zzdi unused) {
            }
            alq alqVar = new alq();
            if (this.d > 0 && aya.a(this.q)) {
                alqVar.m = Long.valueOf(aya.a(this.k, this.q));
                alqVar.n = Long.valueOf(aya.a(this.n - this.l, this.q));
                alqVar.o = Long.valueOf(aya.a(this.o - this.m, this.q));
                alqVar.r = Long.valueOf(aya.a(this.l, this.q));
                alqVar.s = Long.valueOf(aya.a(this.m, this.q));
                if (this.p && this.f2362b != null) {
                    long jA = aya.a(((this.l - this.n) + this.f2362b.getRawX()) - this.f2362b.getX(), this.q);
                    if (jA != 0) {
                        alqVar.p = Long.valueOf(jA);
                    }
                    long jA2 = aya.a(((this.m - this.o) + this.f2362b.getRawY()) - this.f2362b.getY(), this.q);
                    if (jA2 != 0) {
                        alqVar.q = Long.valueOf(jA2);
                    }
                }
            }
            try {
                axz axzVarB = b(this.f2362b);
                alqVar.f2056a = axzVarB.f2395a;
                alqVar.f2057b = axzVarB.f2396b;
                alqVar.h = axzVarB.c;
                if (this.p) {
                    alqVar.c = axzVarB.e;
                    alqVar.e = axzVarB.d;
                    alqVar.g = Integer.valueOf(axzVarB.f.longValue() != 0 ? 1 : 0);
                    if (this.e > 0) {
                        if (aya.a(this.q)) {
                            double d = this.j;
                            double d2 = this.e;
                            Double.isNaN(d);
                            Double.isNaN(d2);
                            lValueOf = Long.valueOf(Math.round(d / d2));
                        } else {
                            lValueOf = null;
                        }
                        alqVar.d = lValueOf;
                        double d3 = this.i;
                        double d4 = this.e;
                        Double.isNaN(d3);
                        Double.isNaN(d4);
                        alqVar.f = Long.valueOf(Math.round(d3 / d4));
                    }
                    alqVar.j = axzVarB.i;
                    alqVar.i = axzVarB.j;
                    alqVar.k = Integer.valueOf(axzVarB.k.longValue() != 0 ? 1 : 0);
                }
            } catch (zzdi unused2) {
            }
            if (this.h > 0) {
                alqVar.l = Long.valueOf(this.h);
            }
            akpVar.K = alqVar;
            if (this.d > 0) {
                akpVar.y = Long.valueOf(this.d);
            }
            if (this.e > 0) {
                akpVar.x = Long.valueOf(this.e);
            }
            if (this.f > 0) {
                akpVar.w = Long.valueOf(this.f);
            }
            if (this.g > 0) {
                akpVar.z = Long.valueOf(this.g);
            }
            try {
                int size = this.c.size() - 1;
                if (size > 0) {
                    akpVar.L = new alq[size];
                    for (int i = 0; i < size; i++) {
                        axz axzVarA2 = a(f2361a, this.c.get(i), this.q);
                        alq alqVar2 = new alq();
                        alqVar2.f2056a = axzVarA2.f2395a;
                        alqVar2.f2057b = axzVarA2.f2396b;
                        akpVar.L[i] = alqVar2;
                    }
                }
            } catch (zzdi unused3) {
                akpVar.L = null;
            }
            arrayList = new ArrayList();
            if (axtVarB.c() != null) {
                int iN = axtVarB.n();
                arrayList.add(new ayo(axtVarB, akpVar));
                arrayList.add(new ayr(axtVarB, "OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", akpVar, iN, 1));
                arrayList.add(new aym(axtVarB, "teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", akpVar, v, iN, 25));
                arrayList.add(new ayl(axtVarB, "55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", akpVar, iN, 44));
                arrayList.add(new ayq(axtVarB, "tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", akpVar, iN, 12));
                arrayList.add(new ays(axtVarB, "M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", akpVar, iN, 3));
                arrayList.add(new ayp(axtVarB, "diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", akpVar, iN, 22));
                arrayList.add(new ayk(axtVarB, "5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", akpVar, iN, 5));
                arrayList.add(new ayz(axtVarB, "ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", akpVar, iN, 48));
                arrayList.add(new ayg(axtVarB, "DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", akpVar, iN, 49));
                arrayList.add(new ayw(axtVarB, "HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", akpVar, iN, 51));
                arrayList.add(new ayv(axtVarB, "dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=", akpVar, iN, 45, new Throwable().getStackTrace()));
                arrayList.add(new aza(axtVarB, "JpT53kCFZ5UGhS3QgNfIxU7Zo+2b3P+3vnYveTkMBHt4zfnotqnDwQTaAD1ryBW9", "SQmHWbTAzPlYuz5I3xZRZNLM/2+IGX009ow9MnjYfrI=", akpVar, iN, 57, view));
                arrayList.add(new ayu(axtVarB, "9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", akpVar, iN, 61));
                if (((Boolean) bra.e().a(o.bf)).booleanValue()) {
                    arrayList.add(new ayf(axtVarB, "TxRdKOZ0pZeBXpIwiLITX81ZqIx50eBVN3DINE4ZBUPdk4novnMzQn3dOLT7/176", "m2+gcfdQWoY+6hOETk2mr0SsbjGMFJBzTmSItOYkEfM=", akpVar, iN, 62, view, activity));
                }
                if (((Boolean) bra.e().a(o.bh)).booleanValue()) {
                    arrayList.add(new ayx(axtVarB, "9MXEPmGC/7986z/wLWv2b+1iRSmCMoPNrO7Sy/hYl1riiNMJKXOd189OeOHpltpa", "DjhY2o1Svq1HpINv7cU+bAqV+OA81bMZ6vkVBk0u3II=", akpVar, iN, 53, this.z));
                }
            }
        }
        a(arrayList);
        return akpVar;
    }

    protected List<Callable<Void>> a(axt axtVar, Context context, akp akpVar, aho ahoVar) {
        int iN = axtVar.n();
        ArrayList arrayList = new ArrayList();
        if (!axtVar.b()) {
            akpVar.k = 16384L;
            return arrayList;
        }
        arrayList.add(new ayj(axtVar, "qsalJ2RwJdF4/xIua4a94Fnnrn+PF/pR/qVYX8QPAaTQDa6NRA9etEnbnhQWblFD", "G68A6YJ+VRJhgHK56Kx1RoGHOyqehPG7tlyyPwtcr7Q=", akpVar, iN, 27, context, ahoVar));
        arrayList.add(new aym(axtVar, "teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", akpVar, v, iN, 25));
        arrayList.add(new ayr(axtVar, "OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", akpVar, iN, 1));
        arrayList.add(new ayt(axtVar, "2a/4JIVPkDFYMOVDvRgiPhzuBvaMjATDrq7QEolObsd/NV02/K8EoZgGgGExyTEN", "zAquriPjpzqGJfvs/9ulqqLxOML+UEqSlyjOz6Tu5VU=", akpVar, iN, 31));
        arrayList.add(new ayy(axtVar, "nm3YsDf+7WO+MEOswfwP/mr6SZsOgwQFOzWfB0E01vRE8SnBNpESCxeGx/E3obzH", "ntbFRCAexgxz9p8O68TxIQjW9W7nZKRNbG91vq8F9ro=", akpVar, iN, 33));
        arrayList.add(new ayh(axtVar, "1IkHdmwdwv9wD8tmJQC+h66q1PQxvS+V9UrDno8I9cxHFnF0E43buTrJO+G+/gQl", "A9jthiAQc+izVBr4mJNKEZTv7NXyHy0tI8Qg667eCzo=", akpVar, iN, 29, context));
        arrayList.add(new ayk(axtVar, "5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", akpVar, iN, 5));
        arrayList.add(new ayq(axtVar, "tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", akpVar, iN, 12));
        arrayList.add(new ays(axtVar, "M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", akpVar, iN, 3));
        arrayList.add(new ayl(axtVar, "55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", akpVar, iN, 44));
        arrayList.add(new ayp(axtVar, "diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", akpVar, iN, 22));
        arrayList.add(new ayz(axtVar, "ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", akpVar, iN, 48));
        arrayList.add(new ayg(axtVar, "DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", akpVar, iN, 49));
        arrayList.add(new ayw(axtVar, "HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", akpVar, iN, 51));
        arrayList.add(new ayu(axtVar, "9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", akpVar, iN, 61));
        return arrayList;
    }

    private static void a(List<Callable<Void>> list) throws InterruptedException {
        ExecutorService executorServiceC;
        if (f2361a == null || (executorServiceC = f2361a.c()) == null || list.isEmpty()) {
            return;
        }
        try {
            executorServiceC.invokeAll(list, ((Long) bra.e().a(o.be)).longValue(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(t, String.format("class methods got exception: %s", aya.a(e)));
        }
    }

    @Override // com.google.android.gms.internal.ads.axb
    protected final axz b(MotionEvent motionEvent) throws zzdi {
        Method methodA = f2361a.a("BDp2p9FkJRWhLjM5/HGlBtfI2hTpjdS5yJQGMqe31LtNjFTgL+0QOyyChfmPPFl0", "got2az0DwXyMWaLpfVutWfEF1l3X50FJhmqzREDcTA8=");
        if (methodA == null || motionEvent == null) {
            throw new zzdi();
        }
        try {
            return new axz((String) methodA.invoke(null, motionEvent, this.q));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdi(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.axb
    protected final long a(StackTraceElement[] stackTraceElementArr) throws zzdi {
        Method methodA = f2361a.a("dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=");
        if (methodA == null || stackTraceElementArr == null) {
            throw new zzdi();
        }
        try {
            return new axr((String) methodA.invoke(null, stackTraceElementArr)).f2382a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdi(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.axb, com.google.android.gms.internal.ads.axa
    public final void a(View view) {
        if (((Boolean) bra.e().a(o.bh)).booleanValue()) {
            if (this.z == null) {
                this.z = new ayc(f2361a, view);
            } else {
                this.z.a(view);
            }
        }
    }
}

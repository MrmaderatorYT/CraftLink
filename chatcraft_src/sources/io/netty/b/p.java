package io.netty.b;

import com.github.steveice10.mc.v1_5.util.Constants;

/* compiled from: HeapByteBufUtil.java */
/* loaded from: classes.dex */
final class p {
    static byte a(byte[] bArr, int i) {
        return bArr[i];
    }

    static short b(byte[] bArr, int i) {
        return (short) ((bArr[i + 1] & Constants.DimensionIds.NETHER) | (bArr[i] << 8));
    }

    static short c(byte[] bArr, int i) {
        return (short) ((bArr[i + 1] << 8) | (bArr[i] & Constants.DimensionIds.NETHER));
    }

    static int d(byte[] bArr, int i) {
        return (bArr[i + 3] & Constants.DimensionIds.NETHER) | ((bArr[i] & Constants.DimensionIds.NETHER) << 24) | ((bArr[i + 1] & Constants.DimensionIds.NETHER) << 16) | ((bArr[i + 2] & Constants.DimensionIds.NETHER) << 8);
    }

    static int e(byte[] bArr, int i) {
        return ((bArr[i + 3] & Constants.DimensionIds.NETHER) << 24) | (bArr[i] & Constants.DimensionIds.NETHER) | ((bArr[i + 1] & Constants.DimensionIds.NETHER) << 8) | ((bArr[i + 2] & Constants.DimensionIds.NETHER) << 16);
    }

    static long f(byte[] bArr, int i) {
        return (bArr[i + 7] & 255) | ((bArr[i] & 255) << 56) | ((bArr[i + 1] & 255) << 48) | ((bArr[i + 2] & 255) << 40) | ((bArr[i + 3] & 255) << 32) | ((bArr[i + 4] & 255) << 24) | ((bArr[i + 5] & 255) << 16) | ((bArr[i + 6] & 255) << 8);
    }

    static void a(byte[] bArr, int i, int i2) {
        bArr[i] = (byte) i2;
    }

    static void b(byte[] bArr, int i, int i2) {
        bArr[i] = (byte) (i2 >>> 8);
        bArr[i + 1] = (byte) i2;
    }

    static void c(byte[] bArr, int i, int i2) {
        bArr[i] = (byte) (i2 >>> 24);
        bArr[i + 1] = (byte) (i2 >>> 16);
        bArr[i + 2] = (byte) (i2 >>> 8);
        bArr[i + 3] = (byte) i2;
    }

    static void a(byte[] bArr, int i, long j) {
        bArr[i] = (byte) (j >>> 56);
        bArr[i + 1] = (byte) (j >>> 48);
        bArr[i + 2] = (byte) (j >>> 40);
        bArr[i + 3] = (byte) (j >>> 32);
        bArr[i + 4] = (byte) (j >>> 24);
        bArr[i + 5] = (byte) (j >>> 16);
        bArr[i + 6] = (byte) (j >>> 8);
        bArr[i + 7] = (byte) j;
    }
}
